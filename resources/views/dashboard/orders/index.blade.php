            ...
            <div class="d-flex flex-stack flex-wrap mb-15">
                ...
                <div class="d-flex align-items-center position-relative my-1 mb-2 mb-md-0">
                    ...
                    <select class="form-select form-select-solid w-200px ms-4 border-gray-300 border-1 filter-datatable-inp"
                        ...
                        data-placeholder="{{ __('Status') }}" id="statusFilter">
                        <option></option>
                ...
                <!-- begin :: Toolbar -->
                <div class="d-flex gap-3" data-kt-docs-table-toolbar="base">
                    ...
                    <a href="{{ route('dashboard.orders-export') }}?status=" + selectedStatus class="btn btn-success"
                        data-bs-toggle="tooltip" title="" id="exportExcelBtn">
...
@push('scripts')
    ...
    <script>
        ...
        // Callback function to format the date range
        function cb(start, end) {
            fromToDp.html(start.format("YYYY-MM-DD") + "  -  " + end.format("YYYY-MM-DD"));
        }
        ...
        fromToDp.daterangepicker({
            ...
            endDate: end,
            locale: {
                format: 'YYYY-MM-DD' // Correct format for dates
            },
            ranges: {
        ...
        // Translate 'Custom Range' to the correct language
        $('li[data-range-key="Custom Range"]').html(translate('Custom Range'));
    </script>

    <script>
        // Listen for changes in status or date range
        $('#statusFilter, #from_to_dp').on('change', function() {
            const selectedStatus = $('#statusFilter').val(); // Get the selected status
            const selectedDateRange = fromToDp.text().trim(); // Get the selected date range (formatted)

            const exportButton = $('#exportExcelBtn'); // Export button ID
            const baseUrl = "{{ route('dashboard.orders-export') }}"; // Base export route
            ...
            if (selectedDateRange && selectedDateRange !== "") {
                if (url.includes('?')) {
                    url += `&date_range=${encodeURIComponent(selectedDateRange)}`;
                } else {
                    url += `?date_range=${encodeURIComponent(selectedDateRange)}`;
                }
            }
