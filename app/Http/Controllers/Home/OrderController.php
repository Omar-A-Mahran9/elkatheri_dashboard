...
class OrderController extends Controller
{
    public function create()
    {
        ...
        $fundingOrganizations = FundingOrganization::select('id', 'name_' . getLocale(), 'offer_' . getLocale(), 'image', 'status')->get();
        
        return view('web.purchase-car', compact('cars', 'cities', 'banks', 'fundingOrganizations', 'type'));
    ...
    public function individualsFinance(Request $request){
        ...
        $ordersTableData = $request->validate([
            ...
            'funding_organization_id' => ['bail', 'required_without_all:bank_id,funding_organization_name', 'nullable', Rule::exists('funding_organizations', 'id')],
            'funding_organization_name' => ['bail', 'required_without_all:funding_organization_id,bank_id'],
            'bank_id' => ['bail','required_without_all:funding_organization_id,funding_organization_name', 'nullable', Rule::exists('banks', 'id')],
        ]);
    ...
    public function requestTestDrive(Request $request){
        
        $car = Car::select('id')->where('id', $request->car_id)->first();
        if(!$car)
        {
            throw ValidationException::withMessages([
                'car_id' => __("You must select a car")
        ...
        $testDrive = TestDrive::where('car_id', $request->car_id)->where('phone', $request->phone)->where('date', $request->date)->first();
        if($testDrive)
        {
            throw ValidationException::withMessages([
                'test_drive' => __("You've already requested a test at this date")
            ]);
        }
        ...
        $this->sendEmailToAdmin($testDrive);
    ...
    public function sendEmailToAdmin($order)
    {
        ...
        // });
        Mail::to([settings()->get('email')])->send(new NewOrder($order));
    }
