...
class FundingOrganizationController extends Controller
{
    ...
    public function update(Request $request, FundingOrganization $fundingOrganization)
    {
        ...
        $data = $request->validate([
            ...
            'name_ar'           => ['required','string','max:255','unique:funding_organizations,name_ar,' . $fundingOrganization->id ],
            'name_en'           => ['required','string','max:255','unique:funding_organizations,name_en,' . $fundingOrganization->id ],
            'offer_ar'          => ['required','string'],
            'offer_en'          => ['required','string'],
        ]);
